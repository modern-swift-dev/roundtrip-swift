...
Construct ``NetworkService`` with a `URLSessionConfiguration` whose `protocolClasses` contains a test `URLProtocol` subclass. Pass that service to ``RestClient``. See the [RoundTrip URL loading stub](https://modern-swift-dev.github.io/docs/roundtrip-swift/api/roundtrip/documentation/roundtrip/testing/) for a complete example.
